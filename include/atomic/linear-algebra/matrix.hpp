...
namespace atomic {
    namespace linalg {
        ...
        constexpr matrix<ENG>::matrix(const storage_type data)
                : m_engine(data) {
            atomic::detail::trace("Created matrix", rows(), "x", columns(), " with storage_type: ",
                                  typeid(data).name());
        }
        ...
        constexpr matrix<ENG>::matrix(const ENG &engine)
                : m_engine(engine) {
            atomic::detail::trace("Created matrix", rows(), "x", columns(), " with engine_type: ",
                                  typeid(engine).name());
        }
        ...
        inline auto operator+(const matrix<ENG1> &lhs, const matrix<ENG2> &rhs) {
            atomic::detail::assertion(lhs.dimensions() == rhs.dimensions(),
                                      "Matrices must be equal sizes to perform addition.");
        ...
        inline auto operator-(const matrix<ENG1> &lhs, const matrix<ENG2> &rhs) {
            atomic::detail::assertion(lhs.dimensions() == rhs.dimensions(),
                                      "Matrices must be equal sizes to perform subtraction.");
