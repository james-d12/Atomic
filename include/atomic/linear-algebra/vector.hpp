...
namespace linalg {
  ...
  using fvector = vector<fixed_vector_engine<T, D>>;
  template<typename T>
  using fvector2 = vector<fixed_vector_engine<T, 2>>;
  template<typename T>
  using fvector3 = vector<fixed_vector_engine<T, 3>>;
  template<typename T>
  using fvector4 = vector<fixed_vector_engine<T, 4>>;
  template<typename T>
  ...
  template<class ENG, typename S, typename = std::enable_if_t<is_scalar_element<S>::value>>
  ...
  inline auto operator==(const vector<ENG1> &lhs, const vector<ENG2> &rhs)
  {
    return (lhs.dimensions() != rhs.dimensions())
             ? false
             : detail::vector_engine_equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
  }
  ...
  inline auto operator>(const vector<ENG1> &lhs, const vector<ENG2> &rhs)
  {
    return (lhs.dimensions() != rhs.dimensions())
             ? false
             : detail::vector_engine_less_than(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
  }
